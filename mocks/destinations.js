...
const getRandomDescription = () => {
  const availableSentences = wholeDescription.split(`.`).filter((x) => x !== ``);
  const sentenceCount = generateRandomInt(1, 5);
  const sentenceSet = new Set();
  while (sentenceSet.size !== sentenceCount) {
    const randomSentence = pickRandomElement(availableSentences);
    sentenceSet.add(randomSentence);
  }

  return Array.from(sentenceSet).join(`.`);
};
